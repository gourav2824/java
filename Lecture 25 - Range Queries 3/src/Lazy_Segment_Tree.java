...
public class Lazy_Segment_Tree {
	...
	public static void main(String[] args) {
		...
		rangeUpdate(0, 0, ba.length - 1, 5, 9, 4);
		rangeUpdate(0, 0, ba.length - 1, 2, 6, -2);
	...
	private static void rangeUpdate(int si, int ss, int se, int us, int ue, int delta) {
		
		handlingPendingUpdates(si, ss, se);
		...
		if(se < us || ss > ue) {
			...
			int mid = (ss + se) / 2;
			rangeUpdate(2 * si + 1, ss, mid, us, ue, delta);
			rangeUpdate(2 * si + 2, mid + 1, se, us, ue, delta);
			sa[si] = sa[2 * si + 1] + sa[2 * si + 2];
		}
	}
	...
	private static void handlingPendingUpdates(int si, int ss, int se) {
